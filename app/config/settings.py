...
class Settings(BaseSettings):
    ...
    # 触达频次下限(**买家级**,投递侧仲裁):同一买家两条营销消息之间的最小
    # 间隔小时数。0 = 关闭这条规则。
    #
    # **为什么这条默认开、而上面那条店铺级静默默认关**——两者都是"策略",但错
    # 的代价不在一个量级:
    #   - 工单阈值确实因店而异(大店 5 条未结是常态、小店是事故),替店主拍这个
    #     数就是替他做经营决策,所以机制在、策略关;
    #   - "别在一小时内给同一个人连发两条推销"没有合理的因店而异空间,而且它防
    #     的是**不可逆的对外骚扰**:消息发出去收不回来,少发一条下一轮还能发。
    # 默认值要放在**漏配代价最小**的那一侧,这两条落在相反的两侧,所以默认相反。
    #
    # 与跟进序列不冲突:`followup_interval_hours` 是 48,远大于这个下限。
    outreach_min_interval_hours: float = 1.0
    # 触达**承接窗口**(小时):买家在这段时间内的回复会被当成「在回应那条触达」,
    # 于是把触达前情(商机情境/关联订单/券码)注进买家侧 prompt,并参与路由回落。
    #
    # 与上面那条是两件事,别混:那个是「多久内不许再发」(防骚扰下限,1 小时),
    # 这个是「多久内的回复算承接」(上下文有效期,24 小时)。
    #
    # 为什么默认开、而且给得比较宽:漏配的代价正是要修的那个缺陷——买家回一句
    # 「好啊」时客服不知道前情、要顾客重复说明来意。而配得过宽的代价只是客服多
    # 知道一件已经过去的事,它拿到的全部信息都是确定性映射出来的、不含任何内部
    # 经营数据(见 outreach_context 模块 docstring)。0 = 关闭承接。
    outreach_context_window_hours: float = 24.0
    # 商品级推广静默时长(小时):某商品出异常后,多久之内不对涉及该商品的买家
    # 发营销触达。0(默认)= **不写静默记录**,即这条策略关闭。
    #
    # 与上面两条是第三条轴,三者互补不重复:
    #   店铺级(工单数) / 买家级(接管中·未结工单·频次) / 商品级(该商品在出问题)
    #
    # 为什么默认关:同上,时长是业务政策。但**扇出本身与开关无关**——guard 分支
    # 照样认领事件、照样在协作时间线上留痕(见 collab.handle_guard),关的只是
    # "写不写一条会真的拦人的记录"。这样"事件广播"这个机制的可观测性不依赖于
    # 某个部署方是否采纳了这条策略。
    collab_promotion_pause_hours: int = 0
