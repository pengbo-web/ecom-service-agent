...
#
# **漏一个词的代价是实测过的。** 走查时发「我要找人工客服」没有命中,于是绕过了
# `_human_request_flow` 那句审过的固定话术,落到模型生成——模型回了
# 「请稍候,系统将在**30秒内**为您接入专属服务人员」,凭空许了一个 SLA(而队列里
# 当时积压着 42 条待接管)。也就是说:**这个匹配器每漏一种说法,就多一次编造承诺
# 的机会**,漏的不是一次短路优化,是一道话术护栏。
#
# 尤其刺眼的是 `转接人工` 之前也漏——那正是系统自己那句固定话术里的用词
# (「正在为您转接人工客服」),买家照着系统的说法讲反而匹配不上。
#
# 扩词的纪律不变:整句锚定 + `match_human_fast` 的 ≤10 字上限,所以
# "人工审核要多久""人工客服几点上班"这类正常问句仍然不会命中(它们不是纯请求)。
HUMAN_REQUEST_PATTERNS = [
    re.compile(
        r"^(我?[要想]?(找|叫|转|接)?"
        r"(转人工|转接人工|人工客服|人工服务|人工|真人客服|真人)"
        r"|叫?真人客服?"
        r"|(要|找|叫)真人"
        r")[\s!！~。.,，]*$"),
]
