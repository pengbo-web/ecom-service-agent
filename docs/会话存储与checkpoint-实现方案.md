...
> **进度(2026-07-22)**:**R1–R6 已完成**(存储抽象+Redis / 步级 checkpoint+恢复 / 挂起动作持久化 / 分布式锁 / 冷归档+AOF+compose / 写工具幂等)。真 Redis + fakeredis 均验证,离线 307 绿。
> R6 范围说明:实现了**写工具幂等层**(重试/重放/续跑同一写操作不重复副作用,`applied:{key}` 缓存成功结果,仅 redis 后端启用)——这是"可安全续跑"的基石。**未做**"LLM 驱动的 ReAct 回合全自动续跑":因 LLM 非确定性,重跑会问出不同工具序列、无法精确复现,强做不安全;L1 恢复(不丢上下文/挂起)+ 幂等层已覆盖真实安全需求。
