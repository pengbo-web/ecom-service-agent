...
4. **「必须与用户确认退款原因」只是弱强制**：声明里能表达的只有 `reason` 非空且 `min_length: 2`。守卫只看得到**工具参数**，看不到对话历史，因此无法区分「真的与用户确认过」和「模型自己编了个两字理由」。对外只能说「**前置步骤与参数格式**由代码强制」，**不能**说「确认过程由代码强制」。
5. **守卫作用域是"本轮加载过的 skill"，且前置条件只在本轮内计算**：一轮内加载多个 skill 时，会对**每个**都判（这点已修，早期实现里后加载的 skill 会把前一个的守卫顶掉）；但跨轮不累积——上一轮查过的订单不算这一轮的前置条件。这是刻意的（跨轮记账会让"顾客换了单号"这类情况漏防），但意味着模型必须在同一轮内完成"查单→退款"。
...
**可以如实说**：「**退货退款流程**采用声明式工作流守卫：前置步骤未完成、参数格式未校验通过时，工具调用被系统拦回，模型须补齐后重试——顺序与参数合规由代码强制，不依赖模型自觉。低风险咨询类保持指令式以保留灵活性（对齐 Agent Skills 开放标准）。」

**不能说**：
- 「全流程状态机驱动」/「完全确定性执行」——步骤编排仍在 ReAct 里，只有前置条件是硬的。
- 「高风险流程（退款/**取消**/**改址**）都有守卫」——目前**只有 `process-return` 一个 skill 声明了 workflow 块**。`cancel_order` / `change_address` 没有任何声明式守卫（它们仍受 `consent.py` 的授权门保护，但那管的是"用户批没批"，不管"前置步骤做没做"）。要覆盖它们需要各自补声明。
- 「退款原因由代码确保已与用户确认」——见已知边界 4，只强制了非空与最小长度。
