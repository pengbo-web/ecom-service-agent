...
class FakeAgent:
    ...
    R3:挂起动作放在 agent._pending(随会话持久化),streaming 从这里读/清。
    """
...
def _events(user_input):
    agent = FakeAgent(pending=_refund_pending())
...
def test_confirm_replays_refund_deterministically():
    ...
    # 挂起已清除(agent._pending 置空),写回历史并保存
...
def test_non_confirmation_does_not_replay():
    ...
    assert agent._pending is not None                          # 挂起未被消费
